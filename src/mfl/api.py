...
class Api:
    ...
    def __init__(self, year):
        ...
        self.mfl_export_url = 'https://www77.myfantasyleague.com/{}/export'.format(year)
    ...
    def players(self, players=None, since=None, details=False):
        ...
        if since is not None:
            timestamp = self.convert_to_timestamp(since)
            params['SINCE'] = timestamp
        if players is not None:
            params['PLAYERS'] = self.concat(players)
    ...
    def adp(self,
        ...
        if is_keeper:
            ...
            params['IS_KEEPER'] = 0
        if time is not None:
            params['TIME'] = self.convert_to_timestamp(time)
        if days is not None:
            params['DAYS'] = days
        return self._export(params)
